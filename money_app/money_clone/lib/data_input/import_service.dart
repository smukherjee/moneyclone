...
class ImportService {
  ...
  Future<String> recognizeText(File imageFile) async {
    try {
      ...
      _logger.error('Error recognizing text', e);
      return '';
    }
  }
  ...
  List<Map<String, dynamic>> parseReceiptText(String text) {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (amountMatches.isNotEmpty) {
        final amount =
            amountMatches.first.group(0)?.replaceAll('\$', '') ?? '0.00';
        ...
        final title =
            i > 0 && !amountPattern.hasMatch(lines[i - 1])
                ? lines[i - 1].trim()
    ...
    try {
      ...
      for (int i = 1; i < rows.length; i++) {
        ...
        try {
          ...
          if (dateIndex == null || descIndex == null || amountIndex == null) {
            continue;
          }
          ...
          if (amountCell.value is num) {
            ...
            final amountStr = amountCell.value.toString().replaceAll(
              RegExp(r'[^\d\.-]'),
              '',
            );
          ...
          if (kDebugMode) {
            _logger.error('Error parsing Excel row $i', e);
          }
          continue;
        }
      }
      ...
      if (kDebugMode) {
        _logger.error('Error importing from Excel', e);
      }
      return [];
    }
  }
